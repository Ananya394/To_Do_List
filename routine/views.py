...
def activity_board(request):
    ...
    for activity in activities:
        ...
        if not activity.date:
            columns['no_date'].append(activity)
        elif activity.date == today:
            columns['today'].append(activity)
        elif activity.date <= next_week:
            columns['next_week'].append(activity)
        else:
            columns['later'].append(activity)

    form = ActivityForm()
    return render(request, 'routine/activity_board.html', {'columns': columns, 'form': form})
